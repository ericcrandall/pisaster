...
```{r}
wd<-"/Users/eric/Datasets/pisaster/migrate_second_round/final_runs"
setwd(wd)
...
data.list<-list() #initialize an empty data list
  ...
  for(r in 1:3){  #loop through the 3 replicate rounds (6,7,8)
    wd1<-file.path(wd,paste("rep",r,sep=""),"Stepping Stone Symmetric")
...
#convert each dataframe to an mcmc object and then convert the whole thing to an mcmc list
  ...
  data.list.gg<-ggs(data.list.plot,burnin=F,description="Pisaster ochraceus Stepping Stone Symmetric")
...
steporder<-read.csv("./step_edges_migrate.csv",header=F)
...
cat("Plotting Nm Violin Plots") 
  ...
  popnames<-c("Patrick's Point","Kibesillah","Walk On Beach","Davenport","Fanshell","Soberanes","Vista Del Mar","Hazard Reef")
  ...
  #remove northward names because best model is symmetrical
  stepnames<-stepnames[-c(1,3,5,7,9,11,13)]
  ...
  violin<-ggplot(longNm, (aes(x=rev(Parameter), y=Nm))) + geom_violin(draw_quantiles=c(0.025,0.5,0.975))  + 
